...
repeatNUM = 10  # how many repeats needed
...
for (repeat_i in 1:repeatNUM) { 
  ...
  if (useBulk == TRUE) {
    ...
    if (useHigh_variable_gene == TRUE) { # select highly variable genes (from bulk RNA-seq)
      x_train_temp = logcounts_b_bulk[, train_ind]
      x_train_mean = rowMeans(x_train_temp, na.rm = TRUE)
      x_train_var = apply(x_train_temp,1,function(x) var(x,na.rm = TRUE))
      ...
      x_train0 = x_train_temp[varorder[1:high_variable_gene_num],] # filter for top 2000 genes
    }
  }
  for (cell_i in 1:cellTypeNUM) { # cellTypeNUM
    ...
    if (useBulk == FALSE){
      ...
      if (useHigh_variable_gene == TRUE) { # select highly variable genes (from same cell-type RNA-seq)
        x_train_temp = logcounts_b_list[[cell_i]][, train_ind]
        x_train_mean = rowMeans(x_train_temp, na.rm = TRUE)
        x_train_var = apply(x_train_temp,1,function(x) var(x,na.rm = TRUE))
        coef_var_square = x_train_var/x_train_mean^2 # coefficient of variation, measures relative dispersion of data around the mean
        min_mean = unname( quantile( x_train_mean[ which( coef_var_square > .3 ) ], .95 ) )
    ...
    PCC_all = cor(t(x_train0), t(y_train), use='complete.obs')
    y_train = t(y_train)
    ...
    #for (gene in gene_names[1:2]) {
      ...
      x_train_1gene = x_train[,top50]
      x_test_1gene = x_test[,top50]
...
save(results_repeats,
     file = paste0(pred_table_dir, "Mean_results_repeats_lasso_", filter_cell_num, ".RData"))
```
